...
describe ICMPPacket, "when read from a pcap file" do
  before :all do
      parsed_packets = PcapFile.read_packets(File.join(PCAPS_LOCATION,"sample.pcap"))
      @icmp_packet = parsed_packets[3]

      parsed_packets3 = PcapFile.read_packets(File.join(PCAPS_LOCATION,"sample3.pcap"))
  ...
  context "when reading/writing ICMPPacket to disk" do
    ...
    it "should read a PCAP file from disk" do
      sample_packet = PcapFile.new.file_to_array(:f => "#{PCAPS_LOCATION}/sample.pcap")[2]
