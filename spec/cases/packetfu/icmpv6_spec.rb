...
describe ICMPv6Packet, "when read from a pcap file" do
  before(:all) do
    parsed_packets = PcapFile.read_packets(File.join(PCAPS_LOCATION, "ipv6_icmp.pcap"))
  ...
  context "when reading/writing ICMPv6Packet to disk" do
    ...
    it "should read a PCAP file from disk" do
      sample_packet = PcapFile.new.file_to_array(:f => "#{PCAPS_LOCATION}/ipv6_icmp.pcap").first
