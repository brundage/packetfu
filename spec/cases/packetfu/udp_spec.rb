...
describe UDPPacket do
  ...
  context 'when read from a pcap file' do
    context '(UDP over IPv4)' do
      before(:all) do
        @udp4_packet = PcapFile.read_packets(File.join(PCAPS_LOCATION, "ipv4_udp.pcap")).first
      end
    ...
    context '(UDP over IPv6)' do
      before(:all) do
        @udp6_packet = PcapFile.read_packets(File.join(PCAPS_LOCATION, "ipv6_udp.pcap")).first
      end
